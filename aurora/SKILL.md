...
- Respond with a single short line, e.g.:
  ...
  > *Aurora v1.14.0 is already loaded.*
...
- If gh returns a valid version tag (like `v1.7.12`), strip the leading `v` and compare to the installed version `1.14.0`. If the fetched version is semver-greater, output the update notice (see below) BEFORE the banner.
...
```
🔔 A newer Aurora is available: v<latest> (you have v1.14.0).
   Update: claude plugin update aurora@aurora-smart-home
   Then /reload-plugins or restart Claude Code.
```

What's new notice (only when gh succeeded AND fetched version == installed version `1.14.0`):

```
✨ Aurora v1.14.0 — what's new:
   • Two new recipes: daylight-lights (dim lights by measured lux) and
     dc-power-monitor (measure what a DC device draws).
   • Four more verified sensors in the catalog: SHT31, AHT20, BH1750,
     and INA219, each with a real JLCPCB part number and stock status.
   • weather-station and motion-light now point at the new sensors for
     higher accuracy and a real lux reading.
```
...
Then output `v1.14.0 (released 2026-06-13)` on its own line, then output the banner:
