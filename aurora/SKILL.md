...
- Respond with a single short line, e.g.:
  ...
  > *Aurora v1.9.3 is already loaded.*
...
- If gh returns a valid version tag (like `v1.7.12`), strip the leading `v` and compare to the installed version `1.9.3`. If the fetched version is semver-greater, output the update notice (see below) BEFORE the banner.
...
```
🔔 A newer Aurora is available: v<latest> (you have v1.9.3).
   Update: claude plugin update aurora@aurora-smart-home
   Then /reload-plugins or restart Claude Code.
```

What's new notice (only when gh succeeded AND fetched version == installed version `1.9.3`):

```
✨ Aurora v1.9.3 — what's new:
   • Modern HA syntax everywhere: every YAML example in every skill,
     starter template, and all 27 examples now uses triggers:/conditions:/
     actions:, action: calls, and perform-action dashboard taps.
   • Four new reference files: ESPHome packages (modular fleet configs),
     HA Jinja2 macros, HA labels & categories, Node-RED Dashboard 2.0.
   • Deeper troubleshooting: automation debugging with traces, Node-RED
     git-based Projects, and a migration guide that documents the
     2024.8/2024.10 syntax renames step by step.
```
...
Then output `v1.9.3 (released 2026-06-12)` on its own line, then output the banner:
